import pandas as pd
from src.config import MISSING_PART_SPLIT_PATTERN


def analyze_missing_concentration(df: pd.DataFrame) -> pd.DataFrame | None:
...
def analyze_rework_anomalies(df: pd.DataFrame) -> pd.DataFrame | None:
...
def analyze_group_missing(df: pd.DataFrame) -> pd.DataFrame:
...
def analyze_helper_workload(df: pd.DataFrame) -> pd.DataFrame:
...
def compute_style_stats(df: pd.DataFrame) -> pd.DataFrame:
...
def compute_daily_rework_stats(df: pd.DataFrame) -> pd.DataFrame | None:
    daily_rework = df.dropna(subset=['record_date']).copy()
    if daily_rework.empty:
        return None
...
def generate_material_suggestions(df: pd.DataFrame, recent_days: int = 30) -> pd.DataFrame | None:
    if df.empty:
        return None
...
def get_pending_materials(df: pd.DataFrame) -> pd.DataFrame | None:
