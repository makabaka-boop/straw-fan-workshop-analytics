...
def create_completion_rate_chart(style_stats):
    fig = px.bar(
        style_stats,
        y='style_name',
        x='完成率',
...
def create_rework_trend_chart(daily_stats):
    ...
    fig.add_trace(
        go.Scatter(
            x=daily_stats['日期'], y=daily_stats['返工率'],
            name='返工率(%)', mode='lines+markers',
...
def create_missing_parts_chart(missing_analysis, top_n=10):
...
def create_helper_workload_chart(helper_stats):
    fig = px.bar(
...
def create_helper_rework_rate_chart(helper_stats):
    fig = px.bar(
        helper_stats.reset_index(),
        x='helper_name',
        y='返工率',
