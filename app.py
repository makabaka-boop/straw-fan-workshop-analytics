...
from config import STANDARD_FIELDS, FIELD_TYPES, DATA_TYPE_OPTIONS
from data_loader import load_csv, detect_data_type
from column_mapping import auto_map_columns
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        st.session_state.data_type = DATA_TYPE_OPTIONS[data_type]
        ...
        if uploaded_file is not None:
            try:
                ...
                if st.button("✅ 确认映射并处理数据", type="primary"):
                    processed = process_data(df, mapping, source_type=st.session_state.data_type)
        ...
        if filtered_df is not None and not filtered_df.empty:
            ...
            date_range_arg = date_range if use_date_filter else None
...
def render_overview_tab(filtered_df):
    ...
    with col_right:
        ...
        daily_rework = filtered_df.dropna(subset=['record_date'])

        if len(daily_rework) > 0:
            daily_rework = daily_rework.copy()
