...
from config.constants import STANDARD_FIELDS, FIELD_TYPES, DATA_TYPE_MAP
    ...
    with st.sidebar:
        ...
        if filtered_df is not None and not filtered_df.empty:
            ...
            filtered_df = filter_data(
                filtered_df,
                date_range=date_range if has_valid_dates else None,
...
def render_overview_tab(filtered_df):
    ...
    with col_left:
        ...
        style_stats = calculate_style_stats(filtered_df)
...
def render_anomaly_tab(filtered_df):
    st.header("⚠️ 异常分析")
...
def render_helper_tab(filtered_df):
    st.header("👥 助理负载分析")
...
def render_pending_tab(filtered_df):
    st.header("📋 待补件组列表")
        ...
        for idx, row in pending_list.iterrows():
            with st.expander(f"📦 {row['group_no']} - {row['style_name']}"):
                ...
                with col3:
                    st.metric(
                        "记录日期",
                        row['记录日期'].strftime('%Y-%m-%d') if pd.notna(row['记录日期']) else '未知'
                    )
...
def render_suggestion_tab(filtered_df):
    st.header("💡 备料建议")
...
def render_full_data_section(filtered_df):
    st.markdown("---")
