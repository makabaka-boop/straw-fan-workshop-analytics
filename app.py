...
from src.config import STANDARD_FIELDS, FIELD_TYPES, DATA_TYPE_LABELS, NUMERIC_FIELDS
from src.mapping import auto_map_columns, detect_data_type
from src.processing import process_data, filter_data, has_valid_dates
...
from src.visualization import (
    create_style_completion_chart,
    create_daily_rework_chart,
...
def render_sidebar():
    with st.sidebar:
        ...
        data_type_label = st.selectbox(
            "数据类型",
            list(DATA_TYPE_LABELS.values()),
            index=3
        )
        label_to_type = {v: k for k, v in DATA_TYPE_LABELS.items()}
        st.session_state.data_type = label_to_type[data_type_label]
        ...
        if filtered_df is not None and not filtered_df.empty:
            ...
            date_range = None
            if has_valid_dates(filtered_df):
...
def render_overview_tab(df):
    ...
    with col1:
        st.metric("总完成数", f"{metrics['total_completed']:,}")
    with col2:
        st.metric("总返工数", f"{metrics['total_rework']:,}")
    with col3:
        st.metric("返工率", f"{metrics['rework_rate']:.1f}%")
    with col4:
        st.metric("缺件记录", f"{metrics['missing_count']}")
    with col5:
        st.metric("活跃分组", f"{metrics['active_groups']}")
    ...
    with col_left:
        ...
        fig = create_style_completion_chart(style_stats)
    ...
    with col_right:
        ...
        if daily_stats is not None and len(daily_stats) > 0:
            fig = create_daily_rework_chart(daily_stats)
            st.plotly_chart(fig, use_container_width=True)
        else:
    ...
    if missing_analysis is not None and len(missing_analysis) > 0:
        ...
        with col_m1:
            fig = create_missing_parts_chart(missing_analysis, top_n=10)
            st.plotly_chart(fig, use_container_width=True)
    ...
    if pending_list is not None and len(pending_list) > 0:
        ...
        for idx, row in pending_list.iterrows():
            with st.expander(f"📦 {row['group_no']} - {row['style_name']}"):
                ...
                with col5:
                    note_val = row['备注']
                    if note_val and note_val not in ['', 'nan']:
                        st.info(f"备注: {note_val}")
...
def render_full_data_section(df):
    ...
    with st.expander("📋 查看完整数据"):
        ...
        st.dataframe(
            style_dataframe(df[display_cols], numeric_cols=NUMERIC_FIELDS),
            use_container_width=True
        )
...
def main():
    ...
    with tab5:
        render_suggestions_tab(filtered_df)
