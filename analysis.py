...
from config import (
    MISSING_PARTS_DELIMITER,
    REWORK_RATE_THRESHOLD,
    HIGH_REWORK_RATE_THRESHOLD,
    HIGH_FREQ_MISSING_THRESHOLD,
    MATERIAL_BUFFER_RATIO,
)
...
def analyze_missing_concentration(df):
    ...
    missing_parts_exploded = missing_df.assign(
        missing_part=missing_df['missing_parts'].str.split(MISSING_PARTS_DELIMITER)
...
def generate_material_suggestions(df, recent_days=30):
    ...
    if rework_analysis is not None and len(rework_analysis) > 0:
        high_rework = rework_analysis[rework_analysis['返工率'] > REWORK_RATE_THRESHOLD].head(3)
        for style, row in high_rework.iterrows():
            suggestions.append({
                ...
                '优先级': '高' if row['返工率'] > HIGH_REWORK_RATE_THRESHOLD else '中'
            })
    ...
    if len(style_demand) > 0:
        for style, row in style_demand.head(3).iterrows():
            ...
            buffer = int(total * MATERIAL_BUFFER_RATIO)
            suggestions.append({
                '类型': '热门样式',
                '材料/样式': style,
